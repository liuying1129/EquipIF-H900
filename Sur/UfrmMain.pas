...
const
  CR=#$D+#$A;
  STX='Sample Report';ETX='End';ACK=#$6;NAK=#$15;
  sCryptSeed='lc';//加解密种子
  //SEPARATOR=#$1C;
...
begin
  ...
  begin
    ss:='串口选择'+#2+'Combobox'+#2+'COM1'+#13+'COM2'+#13+'COM3'+#13+'COM4'+#2+'0'+#2+#2+#3+
      ...
      '联机标识前缀'+#2+'Edit'+#2+#2+'1'+#2+'注:用于并机时与其他仪器的联机标识进行区别'+#2+#3+
...
begin
  ...
  ReceiveItemInfo:=VarArrayCreate([0,15],varVariant);

  dlttype:='Date';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[0]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  CheckDate:='20'+sValue;
  
  dlttype:='Time';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[1]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  CheckDate:=CheckDate+' '+sValue;

  dlttype:='PAT';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[2]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);
  ...
  dlttype:='Cup';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[3]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='Barcode';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),24));
  ReceiveItemInfo[4]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);
  
  dlttype:='K';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[5]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='Na';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[6]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='Cl';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[7]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='iCa';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[8]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='TCa';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[9]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='nCa';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[10]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='TCO2';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[11]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='pH';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[12]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='AG';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[13]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='Mg';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[14]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);

  dlttype:='Li';
  sValue:=trim(copy(Str,pos(dlttype,Str)+length(dlttype),12));
  ReceiveItemInfo[15]:=VarArrayof([OnLineIDPrefix+dlttype,sValue,'','']);
